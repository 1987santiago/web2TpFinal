<?php 
    ...
    function changeReservationStatus($reservation_code) {
        ...
        $sql_update_query = "UPDATE reserva SET estado=0  WHERE codigo_reserva = '$reservation_code';";
        
        // Se agrega la misma a un array de reservas recientemente habilitadas 
        // para evitar que se las agregue a reservas impagas y se las pueda eliminar
        $reservation_enabled = ( isset($_SESSION['reservation_enabled'])? $_SESSION['reservation_enabled'] : array() );
        array_push($reservation_enabled, $reservation_code);
        $_SESSION['reservation_enabled'] = $reservation_enabled;
